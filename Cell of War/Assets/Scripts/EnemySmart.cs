...
public class EnemySmart : MonoBehaviour {
	...
	private float hp;
    private float hpBase = 2;
    private float hpScaling = 1.06f;

    private float movementSpeed;
    private float movementSpeedBase = 4f;
	...
	void loadStats() {
		...
		playerMovementSpeed = GM.movementSpeed;
		speedMult = movementSpeed / playerMovementSpeed;
	}
	...
	void Update() {
		if (player != null) {
			if (Vector3.Distance(player.transform.position, this.gameObject.transform.position) < chaseDistance) {
				Transform playerTransform = player.transform;
				Vector3 playerPosition = playerTransform.position;
				Vector3 diff = playerPosition - transform.position;
				rb.velocity = diff.normalized * movementSpeed * 1.5f;
			} else {
				Vector2 playerVelocity = playerRB.velocity;
				rb.velocity = new Vector2(-playerVelocity.x, playerVelocity.y) * speedMult;
			}
		}
	}
